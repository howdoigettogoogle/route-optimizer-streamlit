...
def load_uploaded_stops(uploaded_files: List[io.BytesIO]) -> pd.DataFrame:
    """Read one or more uploaded semicolon-delimited CSV files."""
    ...
    for uploaded_file in uploaded_files:
        try:
            ...
            raise ValueError(f"Could not read {getattr(uploaded_file, 'name', 'uploaded file')}: {exc}") from exc
...
def route_table(order: List[int], stops_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for node_idx in order:
        if node_idx == 0:
            rows.append({"Visit order": 0, "Type": "Start", "Latitude": None, "Longitude": None, "Source file": ""})
        elif node_idx == len(stops_df) + 1:
            rows.append({"Visit order": len(order) - 1, "Type": "End", "Latitude": None, "Longitude": None, "Source file": ""})
        else:
...
st.set_page_config(page_title="Exact Route Optimizer", page_icon="🗺️", layout="wide")

st.title("Exact Route Optimizer")
st.caption("Upload CSV stops, choose start/end, and compute the proven shortest driving-time path with OSRM + OR-Tools.")

with st.sidebar:
    st.header("Inputs")

    uploaded_files = st.file_uploader(
        "Upload semicolon-delimited CSV file(s)",
        type=["csv"],
        accept_multiple_files=True,
        help="Each CSV must contain Latitude and Longitude columns.",
    )
    ...
    run = st.button("Optimize route", type="primary", use_container_width=True)

with st.expander("CSV format expected"):
    st.write("Your CSV must be semicolon-delimited and must contain these columns:")
    st.code("Latitude;Longitude", language="text")
    st.write("Optional columns used in map popups include Vehicle number, Vehicle battery, Last ride, and Status.")

if not run:
    st.info("Upload your CSV file(s), choose the start/end locations, then tap **Optimize route**.")
    st.stop()
...
try:
    ...
    with st.status("Preparing data...", expanded=True) as status:
        df = load_uploaded_stops(uploaded_files)
