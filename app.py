...
def load_stops_from_uploads(uploaded_files) -> pd.DataFrame:
    """Read one or more uploaded semicolon-delimited CSV files and concatenate them."""
    if not uploaded_files:
        raise ValueError("Upload at least one CSV file or use Paste CSV.")
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            df = read_csv_bytes(data, source_name)
            df["__source_file__"] = source_name
            frames.append(df)
        except Exception as exc:
            read_errors.append(f"{source_name}: {exc}")

    if not frames:
        detail = "\n".join(read_errors) if read_errors else "No readable files."
        raise ValueError(f"Could not read any uploaded CSV files. {detail}")

    df_all = pd.concat(frames, ignore_index=True, sort=False)
    cleaned = clean_stops_dataframe(df_all)
    cleaned.attrs["source_files"] = [getattr(f, "name", "uploaded.csv") for f in uploaded_files]
    cleaned.attrs["read_errors"] = read_errors
    return cleaned


def _split_pasted_csv_blocks(csv_text: str) -> List[str]:
    """Split pasted CSV text into blocks.

    Users can paste one CSV, several CSVs separated by a line containing --- ,
    or several CSVs pasted one after another. Repeated header rows are harmless
    because they are dropped during coordinate cleaning.
    """
    text = csv_text.strip()
    if not text:
        return []
...
def load_stops_from_paste(csv_text: str) -> pd.DataFrame:
    """Read one or more semicolon-delimited CSV blocks pasted into the app."""
    blocks = _split_pasted_csv_blocks(csv_text)
    if not blocks:
        raise ValueError("Paste CSV text or upload CSV files.")

    frames: List[pd.DataFrame] = []
    read_errors: List[str] = []
    for idx, block in enumerate(blocks, start=1):
        source_name = f"pasted_csv_{idx}"
        try:
            df = pd.read_csv(io.StringIO(block), delimiter=";", dtype=str)
            df["__source_file__"] = source_name
            frames.append(df)
        except Exception as exc:
            read_errors.append(f"{source_name}: {exc}")
    ...
    if not frames:
        detail = "\n".join(read_errors) if read_errors else "No readable pasted CSV blocks."
        raise ValueError(f"Could not read pasted CSV text. {detail}")

    df_all = pd.concat(frames, ignore_index=True, sort=False)
    cleaned = clean_stops_dataframe(df_all)
    cleaned.attrs["source_files"] = [f"pasted_csv_{i}" for i in range(1, len(blocks) + 1)]
    cleaned.attrs["read_errors"] = read_errors
    return cleaned
...
st.info(
    "Android tip: you can upload several CSVs. If Android will not show or multi-select them, use **Paste CSV** below instead."
)
...
if input_mode == "Upload file":
    uploaded_files = st.file_uploader(
        "Upload one or more semicolon-delimited CSV files",
        # No type filter: Android file pickers can hide .csv files when accept/type filters are present.
        type=None,
        accept_multiple_files=True,
        help="Each CSV must contain Latitude and Longitude columns. Files renamed to .txt also work if the content is semicolon-delimited CSV.",
    )
    st.caption("File not visible? Open your file manager and choose **All files**, or switch to **Paste CSV**. On many Android file pickers, long-press lets you multi-select files.")
    if uploaded_files:
        st.caption(f"Selected {len(uploaded_files)} file(s): " + ", ".join(getattr(f, "name", "uploaded.csv") for f in uploaded_files))
else:
    csv_text = st.text_area(
        "Paste one or more semicolon-delimited CSVs",
        height=260,
        placeholder=(
            "Latitude;Longitude;Vehicle number;Vehicle battery;Status\n"
            "48.059123;8.458123;V001;72%;Available\n"
            "---\n"
            "Latitude;Longitude;Vehicle number;Vehicle battery;Status\n"
            "48.061234;8.462345;V002;55%;Available"
        ),
        help="Paste several CSVs by putting a line with --- between them. Repeated header rows are OK.",
    )
...
with st.expander("CSV format"):
    ...
    st.write("You can upload multiple CSV files. For Paste CSV mode, separate different CSV blocks with a line containing `---`.")
...
try:
    if run:
        with st.status("Preparing route...", expanded=True) as status:
            if input_mode == "Upload file":
                df = load_stops_from_uploads(uploaded_files)
            else:
                df = load_stops_from_paste(csv_text)

            dropped_rows = df.attrs.get("dropped_rows", 0)
            source_files = df.attrs.get("source_files", [])
            read_errors = df.attrs.get("read_errors", [])
            status.write(f"Loaded {len(df)} valid stops from {len(source_files)} CSV source(s). Dropped {dropped_rows} rows with invalid coordinates.")
            if read_errors:
                status.write("Some CSV sources could not be read and were skipped: " + "; ".join(read_errors))
...
if result:
    ...
    st.caption("Showing the latest optimized route. Tap Optimize route again after changing inputs.")

    # Use a plain HTML component instead of st_folium. This avoids the map component
    # triggering an immediate Streamlit rerun on mobile and disappearing after render.
    components.html(result["map_html"], height=520, scrolling=True)
    ...
    with st.expander("Route order", expanded=False):
        st.dataframe(result["route_table"], use_container_width=True, hide_index=True)
else:
    st.caption("After you tap Optimize route, the map will remain here instead of disappearing on rerun.")
